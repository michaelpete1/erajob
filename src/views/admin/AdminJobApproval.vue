        ...
        <div v-for="job in filteredJobs" :key="job.id" class="bg-white/95 backdrop-blur-sm p-4 sm:p-5 rounded-xl shadow-lg border-l-4 border-white/30 cursor-pointer hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-sm">
            <div class="flex items-center text-gray-600">
              ...
              <span class="text-xs sm:text-sm">{{ job.agentName }} ({{ job.agentUsername }})</span>
            </div>
            <div class="flex items-center text-green-600 font-semibold">
              ...
              <span class="text-xs sm:text-sm">{{ job.pay }}</span>
            </div>
          </div>
...
const filteredJobs = computed(() => {
    // In a real app, 'Approved' would likely show a list of approved jobs, 
    // but based on the image, all three visible jobs appear to be pending/new requests.
    // We'll filter based on the mock data's 'status' field.
    if (currentTab.value === 'Approved') {
        return allJobs.value.filter(job => job.status === 'Approved');
    } else if (currentTab.value === 'Pending') {
        return allJobs.value.filter(job => job.status === 'Pending');
    }
    return allJobs.value; // Fallback to show all
});

// Admin action functions
const approveJob = (jobId: number) => {
    console.log('Approving job:', jobId);
    // In a real app, this would call an API to approve the job
